...
namespace shockz.msa.fileManagement.Endpoints;
...
public static class DownloadFileEndpoint
{
  public static async Task HandleRoute(HttpContext context)
  {
    ...
    if (config.Store is not ITusReadableStore) {
      return;
    }

    var store = (TusDiskStore)config.Store;
  ...
  private static string GetContentTypeOrDefault(Dictionary<string, Metadata> metadatas)
  {
    if (metadatas.TryGetValue("contentType", out var contentType)) {
